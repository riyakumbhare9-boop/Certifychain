...
contract product {
    ...
    // ✅ ACCESS CONTROL: Map seller/manufacturer codes to their owner addresses
    mapping(bytes32 => address) sellerOwners;
    mapping(bytes32 => address) manufacturerOwners;
    
    // ✅ RELATIONSHIP TRACKING: Map seller codes to their manufacturer
    mapping(bytes32 => bytes32) sellerToManufacturer;
    
    // ✅ RELATIONSHIP TRACKING: Map products to their manufacturer
    ...
    uint256 _sellerNum, bytes32 _sellerManager, bytes32 _sellerAddress) public{
        ...
        // ✅ RECORD WHO OWNS THIS SELLER
        sellerOwners[_sellerCode] = msg.sender;
        // ✅ RECORD MANUFACTURER WHO OWNS THIS SELLER
        manufacturerOwners[_manufacturerId] = msg.sender;
        // ✅ TRACK SELLER-TO-MANUFACTURER RELATIONSHIP
    ...
    uint256 _productPrice) public{
        ...
        // ✅ RECORD WHO OWNS THIS MANUFACTURER
        manufacturerOwners[_manufactuerID] = msg.sender;
        // ✅ TRACK PRODUCT-TO-MANUFACTURER
        productToManufacturer[_productSN] = _manufactuerID;
    }
    ...
    function manufacturerSellProduct(bytes32 _productSN, bytes32 _sellerCode, bytes32 _manufacturerId) public{
        // ✅ AUTO-REGISTER SELLER IF NOT ALREADY REGISTERED
        if(sellerOwners[_sellerCode] == address(0)) {
            // Seller doesn't exist yet, register them
            sellers[sellerCount] = seller(sellerCount, bytes32(0), bytes32(0), _sellerCode, 0, bytes32(0), bytes32(0));
            sellerOwners[_sellerCode] = msg.sender;
            manufacturerOwners[_manufacturerId] = msg.sender;
            sellerToManufacturer[_sellerCode] = _manufacturerId;
            sellerCount++;
    ...
    function queryProductsList(bytes32 _sellerCode) public view onlySellerOwner(_sellerCode) returns(uint256[] memory, bytes32[] memory, bytes32[] memory, bytes32[] memory, uint256[] memory, bytes32[] memory){
    ...
    function querySellersList (bytes32 _manufacturerCode) public view onlyManufacturerOwner(_manufacturerCode) returns(uint256[] memory, bytes32[] memory, bytes32[] memory, bytes32[] memory, uint256[] memory, bytes32[] memory, bytes32[] memory) {
